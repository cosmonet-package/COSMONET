...
MarginalCoxRanking <- function(x, y){
  ...
  if(length(which(colSums(x)!=0))==dim(x)[2]){x <- x} else {x <- x[, which(colSums(x)!=0)]}
  ...
  for(m in 1:p){
    beta[m] <- coxph(Surv(times,status)~x.stand[,m], data=x.stand)$coefficients
    pvals[m] <- broom::tidy(coxph(Surv(times,status)~x.stand[,m], data=x.stand))$p.value
    pbar$step()
    }

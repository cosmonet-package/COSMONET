...
#' @param x1 a matrix of intensities, rows are genes and columns are samples \code{pxn1}. 
#' @param x2 a matrix of intensities, rows are genes and columns are samples \code{pxn2}.
...
#' \item{x1.norm}{a normalized matrix \code{n1xp}}
#' \item{x2.norm}{a normalized matrix \code{n2xp}}
#' 
...
NormalizeBetweenData <- function(x1, x2, norm.method="quantile", plot=FALSE){
  ...
  if(plot){ 
    ...
    plot(density(x1.norm[1,]),col=colramp[1],lwd=3,ylim=c(0,.25),main="Training set")
    for(i in 2:20){lines(density(x1.norm[i,]),lwd=3,col=colramp[i])}
    plot(density(x2.norm[1,]),col=colramp[1],lwd=3,ylim=c(0,.25),main="Testing set")

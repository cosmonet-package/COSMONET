...
To incorporate the *a-priori biological information* in the penalized regression model, we need to build the network matrix, i.e., a weighted and undirected graph. This network generally describes relation among molecules, such as protein-protein interaction, gene co-expression, functional similarities, regulatory relations or biochemical reactions. In our analysis to construct the molecular network we used`CreateNetworkMatrix()` function (Figure 1, step ${\bf (c)}$) which implements the following steps:

1. we consider the associated disease-genes, generally collected by the users, in order to have a gene subset of cardinality $|\mathcal{I}|=d\leq p$;
2. we create the *Functional Linkage Matrix*, $FL=(F_{ij})_{(i,j) \in \mathcal{I}}$. This matrix is a probability matrix where each element represents how strong is the relationship between two adjacent genes;
3. we complete the network matrix by adding zero weights for all the genes that have no apparently association with the disease in order to obtain a $p \times p$ matrix. We called this matrix *Omega*.
...
A simple way of generating a *Co-Expression Matrix* is also through the *correlation*, i.e., correlating all pairs of variables (genes) in a correlation matrix. Another possibility to create the *a-priori network* is to use [`STRINGdb package`](http://www.bioconductor.org/packages/release/bioc/html/STRINGdb.html) which provides a R interface to the [STRING](http://www.string-db.org) protein-protein interactions database.

### Variable screenings for Cox model {.unnumbered}

1. **Biomedical-Driven Screening (BMD-screening)**. In this type of screening, we identify the subset of the screened variables $\{x_j, j \in I\}$ by using only the biological information. Typically, this knowledge either derives from the literature (protein-protein interaction, pathway information, etc.) or is collected by users (for example a subset of disease-genes involved in a specific tumor). For our example, we considered as *screened genes* the top 500 genes associated with cancer disease and `ScreeningMethod()` function is applied (Figure 1, step ${\bf b}$).
...
2. **Data-Driven Screening (DAD-screening)**. In this type of screening, we identify the subset of the screened variables $\{x_j, j \in I\}$ by reducing the feature space from a large scale dimension $p$ to a relatively moderate scale $d < p$ by using only information from the data. We apply `MarginalCoxRanking()` function to identify the candidate genes that are highly-correlated with the patient survival according to their corresponding Cox marginal utilities (Figure 1, step ${\bf b}$). The genes are ordered from the largest to the smallest [Zhao et al., 2012](https://www.sciencedirect.com/science/article/pii/S0047259X11001618), [Fan et al., 2010](https://projecteuclid.org/euclid.imsc/1288099013). This function returns a data frame composed by the ranking of all genes, the corresponding regression coefficients and the relative $p$-values. In order to select the optimal threshold $d$ and optimize data prediction, we can choose to rank:

   a. by `regression coefficients`. We select the genes with $\vert \boldsymbol{\beta}_{\mathcal{I}} \vert > \delta_n$, where $\delta_n$ is a fixed threshold that indicates the $d$ top ranked covariates;
   b. by `p.values`. We identify as $d'$-genes all the genes that have $p$-values $< 0.05$;
   c. by both `regression coefficients` (a) and `p.values` (b).

We define the subset of selected genes $\{x_j, j \in I\}$ as those that are highly correlated with patient survival based on the ranking selected by users (a, b or c). Here, we considered as *screened genes* the top 2500 ranking genes (`thresh`) associated with patient survival and `ScreeningMethod()` function is used (Figure 1, step ${\bf b}$).

```{r echo=TRUE, message=FALSE, warning=FALSE}
### (b) Ranking of genes that are highly correlated with the patient survival 
...
#   nfolds=5
# )
# beta.BMD <- fitTraining$beta
# write.table(beta.BMD, "BMD_RegCoeff.txt", sep = "\t", row.names = TRUE, col.names = FALSE)

# summary.BMD <- fitTraining$summary
# summary.BMD

# df.train.BMD <- fitTraining$df
# head(df.train.BMD)
...
#   nfolds=5
# )
# beta.DAD <- fitTraining$beta
# write.table(beta.DAD, "DAD_RegCoeff.txt", sep = "\t", row.names = TRUE, col.names = FALSE)

# summary.DAD <- fitTraining$summary
# summary.DAD

# df.train.DAD <- fitTraining$df
# head(df.train.DAD)
...
#   nfolds=5
# )
# beta.BMD.DAD <- fitTraining$beta
# write.table(beta.BMD.DAD, "BMD_DAD_RegCoeff.txt", sep = "\t", row.names = TRUE, col.names = FALSE)

# summary.BMD.DAD <- fitTraining$summary
# summary.BMD.DAD

# df.train.BMD.DAD <- fitTraining$df
# head(df.train.BMD.DAD)
```

To summarize the results the last command returns a data frame composed the percentage related to the cutoff (from 20% to 80%), the number of low- and high-risk group, the cutoff and the associated $p$-value (as result of the long-rank test).
